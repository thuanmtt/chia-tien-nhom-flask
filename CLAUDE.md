...
# THỨ TỰ DEPLOY khi schema có bảng/cột MỚI mà code mới cần (vd deleted_events):
# chạy schema.sql TRƯỚC rồi mới deploy code — schema chỉ bổ sung nên vô hại
# với code cũ đang chạy. (Máy này không có psql — dùng psycopg2 đọc .env.)
...
`expense_beneficiaries`, `member_bank_info`, `couples`, `couple_members`, `event_rates`, `settlements`
(đánh dấu "đã chuyển tiền": mỗi dòng khớp ĐÚNG một giao dịch trong kết quả chia theo
(from_name, to_name, amount) — kết quả chia đổi thì hết khớp, coi như chưa đánh dấu;
PK tự nhiên (event_id, from, to, amount), client toggle giữ tối đa 1 mục/giao dịch,
event_store khử trùng lặp trước INSERT).
...
`load_event_children` đọc cả 8 bảng con bằng MỘT câu SELECT json_agg (1 round-trip);
...
lịch sử — với người xem KHÔNG phải owner, email bị MASK Ở THỜI ĐIỂM ĐỌC (`t***@domain`):
cả `actor_name` lẫn email nằm trong text summary (regex — che được cả dòng cũ đã lưu
email đầy đủ); dữ liệu lưu giữ nguyên nên owner thấy đầy đủ. `GET
/api/events/<code>/revisions/<id>` (quyền như xem lịch sử) xem trước MỘT phiên bản:
preview snapshot rút gọn (title, members, ≤100 khoản chi — KHÔNG kèm bankInfo) +
`changes` = diff chiều hiện-tại→snapshot ("khôi phục bản này sẽ: ..."), dùng
`diff_documents(..., max_actions=50)`; mask email như danh sách. Nút "Xem" trong
`#historyModal` mở panel này inline. `POST /api/events/<code>/restore` khôi phục
snapshot (validate lại, log dòng 'restore', optimistic locking 409). Xóa event vẫn là DELETE cứng — revisions mất theo — nhưng NGAY
...
- **"Sự Kiện Của Tôi"**: `GET /api/my-events` (JWT) = event sở hữu ∪ được mời đích danh
  ...
  khi 2 request đồng thời). `GET /api/events/<code>` trả thêm cờ `is_saved` (đã đăng
  nhập và đã lưu) — nút "Theo dõi / Bỏ theo dõi" (`#followEventBtn`, chỉ hiện khi
  không phải owner) dựa vào cờ này; KHÔNG còn auto-lưu khi mở event. `DELETE /api/my-events/<code>` gỡ bookmark, KHÔNG đụng
...
**Frontend** — SPA: `templates/index.html` (markup only, no Jinja, KHÔNG script inline — CSP), `static/app.css`, `static/app.js` (~3.3k lines; jQuery + Bootstrap from CDN), plus hai file nhỏ `static/analytics.js` (gtag init — GA ID phải khớp thẻ gtag/js trong index.html) và `static/speed-insights.js`. `static/auth.js` (`window.AppAuth`) wraps Supabase Auth (email/password + Google) and must be loaded before `app.js`; `app.js`'s boot code runs inside `AppAuth.onReady(...)` so it waits for the initial session check. `static/split.js` holds the pure split-money logic (`SplitLogic`, UMD — used by `app.js` in the browser and by `test_split.js` in Node); `app.js` only binds it to page state and renders. Key state lives in localStorage: `currentEventCode`, `savedEventCodes` (the "Sự Kiện Của Tôi" list — used ONLY when logged out; loaded via `POST /api/events/lookup` theo lô ≤50 mã, tra song song rồi gộp — codes missing get pruned CHỈ khi mọi lô thành công), `bankInfo`. `eventEditKeys` is gone (a one-time `localStorage.removeItem('eventEditKeys')` runs at boot to clean up leftovers from older clients). Logged in: the list comes from `GET /api/my-events` (owned ∪ invited ∪ saved, with an `owned` flag) instead of localStorage; a one-time migration (`migrateLocalSavedEvents`, batches of 50, called from `AppAuth.onReady` and on `appauth:change`) pushes any local `savedEventCodes` to the account via `POST /api/my-events/save` and clears local storage only once that succeeds. The per-event action button reflects `owned`: owners see "Xóa sự kiện" (`.delete-event-btn`, hard delete); everyone else sees "Bỏ theo dõi" (`.unsave-event-btn`, icon `fa-bell-slash` khớp nút Theo dõi trên header — `DELETE /api/my-events/<code>` when logged in, local removal when logged out); event `owned` có icon admin (`fa-user-shield text-primary`) trước tên; vào danh sách bằng nút Theo dõi trên header event (không còn auto-lưu `rememberEvent`). Both handlers read the `data-event-code` attribute with `.attr()` rather than `.data()` (jQuery's `.data()` type-coerces all-numeric codes, corrupting them). `loadEventFromServer` sets `allowEdit` from `can_edit` and flips the UI to view-only; a 403 on save does the same. Boot nhanh: khi URL/localStorage có event code, app.js bắn GET event NGAY lúc boot (đọc token supabase-js trong localStorage đồng bộ qua `readCachedSupabaseToken`) song song với `/api/config` + `getSession`; `loadEventFromServer` chỉ nhận nuôi request sớm này khi token lúc bắn === `AppAuth.accessToken()` sau khi auth xong (lệch → request lại như cũ). Chart.js / xlsx / jsPDF / jspdf-autotable KHÔNG nằm trong index.html — được lazy-load trong app.js qua `loadScriptOnce(src, integrity)` (giữ SRI; `ensureChartJs`/`ensureXlsx`/`ensureJsPdf`) khi render biểu đồ hoặc bấm xuất file. Confirmation dialogs go through `showConfirm()` (the shared `#confirmModal`, which must stay LAST among the modals in the DOM so it stacks on top) — don't reintroduce native `confirm()`. `saveEvent` drives the `#saveStatus` header indicator (`setSaveStatus`: saving/saved/error). Kết quả chia: `lastTransfers` giữ transfers của lần tính gần nhất — nút Copy build text từ đây (KHÔNG scrape DOM vì trong `.transfer-item` còn nút QR/đánh dấu) qua `copyTextToClipboard` (có fallback execCommand); nút "Đánh dấu đã chuyển" (`.settle-transfer-btn`, chỉ khi allowEdit; người xem thấy badge) toggle mục trong `settlements` khớp đúng (from, to, amount) rồi `calculateSplit(false)` + `saveEvent(false)`. `sortExpenses` sắp theo thời gian thật (expense_date rồi created_time, giữ thứ tự nhập khi bằng nhau) — không phải thứ tự chèn mảng. Modal `#historyModal` (nút "Lịch sử" trên header, chỉ khi allowEdit) hiển thị revisions + nút khôi phục qua showConfirm; `#confirmModal` vẫn phải là modal CUỐI trong DOM.
...
- **CDN scripts** carry SRI `integrity` hashes (cả các thẻ trong `index.html` LẪN các URL lazy-load trong `app.js` — `ensureChartJs`/`ensureXlsx`/`ensureJsPdf`). Changing a library version requires recomputing: `curl -sfL <url> | openssl dgst -sha384 -binary | openssl base64 -A`.
